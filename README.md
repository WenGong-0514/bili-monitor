...
- 🧹 **显存用完即还 (v5.13.2)** — ASR 模型懒加载, 每次转写结束立即归还本次显存, 空闲 `asr.gpu_idle_release_sec`(默认1800秒)后卸载模型并释放 CUDA 上下文; 实测释放后仅剩 ~180MB 上下文基线, 不占卡时可继续复用 GPU
- 🧠 **本地全流程 (v5.13.2)** — 不做量化，ASR→视觉→文本 三段串行 + 本地对话回复/意图分类（Qwen2.5-VL-7B 视觉 + Qwen3-8B 文本），全链路不依赖云端
