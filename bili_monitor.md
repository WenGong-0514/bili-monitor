...
>
> 最后更新: 2026-08-26
> 版本: v5.13.0 (Docker 容器化部署 + RTX 2080 Ti 本地全流程: ASR→视觉→文本 + 本地对话回复/意图分类 + 意图先行分流 + 总结长度随时长动态调整)
...
| v5.11.1 | 2026-08-26 | **本地对话回复**: 意图分类与评论区对话回复改用本地 Qwen3-8B 生成(不再依赖云端 GLM/DeepSeek); 本地 LLM 常驻复用(视频总结→意图→回复共用同一模型, 避免重复加载), 视觉模型加载前自动释放 LLM 腾显存 |
...
| `dry_populate_unread()` | **v5.6.2新增 启动保护** — 进 main() 主循环前调用一次, 把当前 unread 列表所有 source_id 追加进 state file。防止进程重启/state file 不完整时历史未读触发雪崩回复。只追加不覆盖, 幂等安全 |
...
| `local_pipeline.text_stage(visual_desc, asr_text, ad_segments)` | **v5.11新增** — 本地文本阶段：使用 Qwen3-8B (fp16, 禁用思考) 整合视觉+ASR 生成总结与广告空降提示；返回 `(summary, ad_prefix)`，模型常驻供后续对话复用 |
| `local_pipeline.local_generate(messages, system, max_tokens, temperature)` | **v5.11.1新增** — 常驻 Qwen3-8B 通用文本生成, 供意图分类/对话回复/动态总结复用; `release_llm()` 释放常驻模型显存 |
| `local_pipeline.text_stage(visual_desc, asr_text, ad_segments)` | **v5.11新增** — 本地文本阶段：使用 Qwen3-8B (fp16, 禁用思考) 整合视觉+ASR 生成总结与广告空降提示；返回 `(summary, ad_prefix)`，模型常驻供后续对话复用 |
...
| 函数 | 说明 |
|------|------|
| `main()` | 15秒循环: **启动 dry_populate_unread()** → check_unread → 处理at → 处理reply → sleep |
...
13. **2026-06-26 启动雪崩修复 (v5.6.2)**: 19:10-19:20 事故 — 旧孤儿进程被 kill 后 state file 仅 7 条, 新 systemd 实例启动后B站 unread API 返回累积的历史未读, 全被当新消息处理, 8 条历史 at 被挨个回复。修复: `main()` 启动调用 `dry_populate_unread()` 把当前 unread 全部 source_id 追加进 state file (幂等), 只处理启动后真正新到的消息。注意: 未调B站「标记已读」接口, B站 unread 计数不会自动归零, 仅靠 state file 去重。
